...
public class ProductRepository : IProductRepository
{
    ...
    public bool ProductExists(string name)
    {
        ...
        return _db.Categories.Any(p => p.Name.Trim().ToLower() == name.Trim().ToLower());
    }
    ...
    public ICollection<Product> SearchProducts(string searchTerm)
    {
        ...
        var searchTermLowered = searchTerm.Trim().ToLower();

        if (!string.IsNullOrEmpty(searchTerm))
        {
            query = query.Include(p => p.Category).
                    Where(p => p.Name.Trim().ToLower().Contains(searchTermLowered) 
                    || p.Description.Trim().ToLower().Contains(searchTermLowered) );
        }
